...
- **Compound indexes for sorted keyset** — `(type, data.<sortPath>, entity_id)`. Without one, mongo cannot satisfy the sort from an index, so it fetches every document matching the filter and sorts them in memory: the cost of a page is proportional to the whole matched set, not to the limit. Measured on mongo 7.0.37, the blocking-sort ceiling is 100MB (`internalQueryMaxBlockingSortMemoryUsageBytes = 104857600`) and `allowDiskUseByDefault` is `true`, so exceeding it spills to disk rather than erroring — the failure mode is a silent slowdown, not an alert. Unsorted keyset needs nothing new: it rides `EnsureEntities`'s `(type, entity_id)`.
